...
*/
//Qt5 To fix this issue, you should return a proper Qt::Alignment value instead of an integer. If you want to return a default alignment, you can use Qt::AlignLeft or any other suitable alignment value. Here's how you can modify the code:
...
Qt::Alignment QCPLayoutInset::insetAlignment(int index) const
{
  ...
  {
    qDebug() << Q_FUNC_INFO << "Invalid element index:" << index;
    return Qt::AlignLeft; // Return a default alignment value
  }
}
...
/*
 * The error you're encountering is because the QSet class in Qt6 does not have a toList member function. In previous versions of Qt, QSet had a toList function, but in Qt6, it was removed. To fix this, you can use the QSet iterator directly instead of trying to convert it to a list.
 * */

QCPItemAnchor::~QCPItemAnchor()
{
  // unregister as parent at children:
  for (QCPItemPosition *child : qAsConst(mChildrenX))
  {
    if (child->parentAnchorX() == this)
      child->setParentAnchorX(0); // this acts back on this anchor and child removes itself from mChildrenX
  }
  for (QCPItemPosition *child : qAsConst(mChildrenY))
  {
    if (child->parentAnchorY() == this)
      child->setParentAnchorY(0); // this acts back on this anchor and child removes itself from mChildrenY
  }
...
void QCPColorScale::setRangeDrag(bool enabled)
{
  ...
  //mAxisRect.data()->setRangeDrag(0);
    //Qt6
    mAxisRect.data()->setRangeDrag(Qt::Horizontal); // Set the appropriate default orientation here
}
...
void QCPGraph::setData(const QVector<double> &key, const QVector<double> &value)
{
  ...
  for (int i = 0; i < n; ++i)
  {
    mData->insert(key[i], QCPData(key[i], value[i]));
  }
}
